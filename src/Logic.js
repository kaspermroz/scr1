...
export const uszereguj = (zadania) => {
  ...
  [...Array(calcHyperPeriod(zadania)).keys()].forEach((t) => {
    ...
    if (indexZadania !== -1) {
      ...
      timeline.push({
        jednostkaCzasu: t,
        nazwaZadania: zadania[indexZadania].nazwa,
      });
    ...
    zadaniaCzasuRzeczywistego = zadaniaCzasuRzeczywistego.map(
      (zadanieCzasuRzeczywistego) => {
        zadanieCzasuRzeczywistego.okres -= 1;
        if (zadanieCzasuRzeczywistego.okres === 0) {
          let index = zadania.findIndex(
            (it) => zadanieCzasuRzeczywistego.nazwa === it.nazwa
          );
          zadanieCzasuRzeczywistego = JSON.parse(
            JSON.stringify(zadania[index])
          );
        }
...
const estimatePriority = (zadaniaCzasuRzeczywistego) => {
  let tempPeriod = calcHyperPeriod(zadaniaCzasuRzeczywistego);
  let p = -1;
